...
#pragma warning disable 0649
 ...
 new InputAction( "Toggle Binding", type: InputActionType.Button, binding: "<Keyboard>/backquote", expectedControlType: "Button" );
#else
        ...
        [SerializeField] [HideInInspector] private bool receiveInfoLogs = true,
...
#pragma warning restore 0649
        ...
        private void Awake()
        {
            ...
            recycledListView.Initialize(this, collapsedLogEntries, indicesOfListEntriesToShow,
                timestampsOfListEntriesToShow, logItemPrefab.Transform.sizeDelta.y);
        ...
        private void LateUpdate()
        {
            ...
            var numberOfLogsToProcess =
                IsLogWindowVisible ? queuedLogEntries.Count : queuedLogEntries.Count - queuedLogLimit;
                        ...
                        else
                        {
                            if (searchbar.parent == searchbarSlotTop)
                            {
                                ...
                                logItemsScrollRectTR.sizeDelta =
                                    logItemsScrollRectOriginalSize - new Vector2(0f, searchbarHeight);
                            }
                        }
                    }
        ...
        private char OnValidateCommand(string text, int charIndex, char addedChar)
        {
            if (addedChar == '\t') // Autocomplete attempt
            {
                if (!string.IsNullOrEmpty(text))
                {
                    ...
                    var autoCompletedCommand =
                        DebugLogConsole.GetAutoCompleteCommand(commandInputFieldAutoCompleteBase, text);
        ...
        private void ProcessQueuedLogs(int numberOfLogsToProcess)
        {
            for (var i = 0; i < numberOfLogsToProcess; i++)
            {
                ...
                lock (logEntriesLock)
                {
                    ...
                    timestamp = queuedLogEntriesTimestamps != null
        ...
        public void AdjustLatestPendingLog(bool autoExpand, bool stripStackTrace)
        {
            lock (logEntriesLock)
            {
                ...
                if (stripStackTrace) // Omit the latest log's stack trace
                {
                    ...
                    queuedLogEntries[queuedLogEntries.Count - 1] =
                        new QueuedDebugLogEntry(log.logString, string.Empty, log.logType);
                }
            }
        }
        ...
        private void RefreshCommandSuggestions(string command)
        {
            ...
            for (var i = 0; i < commandCaretIndexIncrements.Count && caretPos > commandCaretIndexIncrements[i]; i++)
                caretArgumentIndex++;
                ...
                for (var i = 0; i < suggestionsCount; i++)
                {
                    if (i >= visibleCommandSuggestionInstances)
                    {
                        if (i >= suggestionInstancesCount)
                            commandSuggestionInstances.Add(Instantiate(commandSuggestionPrefab,
                                commandSuggestionsContainer, false));
                        else
                    ...
                    if (caretArgumentIndex > 0)
                        ...
                        sharedStringBuilder.Append(commandSuggestionHighlightStart)
                            .Append(matchingCommandSuggestions[i].command).Append(commandSuggestionHighlightEnd);
                    ...
                    if (suggestedCommand.parameters.Length > 0)
                    {
                        ...
                        for (var j = 0; j < suggestedCommand.parameters.Length; j++)
                            if (caretParameterIndex != j)
                                ...
                                sharedStringBuilder.Append(commandSuggestionHighlightStart)
                                    .Append(suggestedCommand.parameters[j]).Append(commandSuggestionHighlightEnd);
                    }
        ...
        internal void Resize(PointerEventData eventData)
        {
            ...
            // Vertical resizing
            var notchHeight = -logWindowTR.sizeDelta.y; // Size of notch screen cutouts at the top of the screen
